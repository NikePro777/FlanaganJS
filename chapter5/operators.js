...
// 5.4.5 for/in
// Вообще то же самое что и for/of  но тлько работающее с обьектами. однако не все так гладко...
// общий вид: for (переменная in обьект) {оператор} причем обьект-это выражение, результатом которого будет обьект
o = { x: 1, y: 2, z: 3 };
for (let p in o) {
  // Присвоить ИМЕНА свойств обьекта o  переменной p
  console.log(o[p]); // Вывести значение каждого свойства
}
// В общем сначала js вычисляет обьект,если он не ноль и не андейфайн то свойство передает переменной. причем каждый раз это делает,т.е. на одном обьекте свойства
// вычисляются заного

(a = []), (i = 0);
for (a[i++] in o); // т.е. еще раз, он присваивает ИМЯ свойства. В данном случае тело пустое, поэтому он к след ячейке (i++ т.е. уже к первой) присваивает имя след свойства
console.log(a); // ['x', 'y', 'z']
// Но в действительности он не перечисляет всех свойств обьекта. он не перечисляет символы, среди строк он проходит только по перечислимым???  скажем toString не перечислимое
// крч проще использовать for/of c Object.keys()
// Если еще не перечисленное в обьекте свойство удалилось - оно не перечислится, а если новое создалось то когда как....
