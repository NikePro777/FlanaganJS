...
// модуль node  импортирует другой модуль вызывая функцию require('имя модуля')
// Если модуль встроен в node или в системе чз диспетчер пакетов, то просто применить надо неуточненное имя без / :
const fs = require("fs"); // встроенный модуль файловой системы
const http = require("http"); // фраймворк http сервера express - сторонний модуль, но был установлен локально

// если импортировать модуль собственного кода, то нужно указать путь к нему:
const starts = require("./main.js");
